...
std::ostream& operator<<(std::ostream& s, const URIAuthority& authority) {
  return s << "{ " << authority.userinfo << ", " << authority.host << ", " << authority.port << " }";
}
std::ostream& operator<<(std::ostream& s, const URI& uri) {
  return s << "{ " << uri.scheme << ", " << uri.authority << ", " << uri.path << ", " << uri.query << ", " << uri.fragment << " }";
}
...
boost::optional<URI> ParseURI(const std::string& uri) {
  ...
  auto opt_authority = ParseAuthority(std::string(match[4].first, match[4].second));
  if (!opt_authority) {
    return boost::none;
  }
